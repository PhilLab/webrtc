...
int32_t VideoRenderWindowsImpl::Init() {
    ...
    switch (_renderMethod) {
        case kVideoRenderWinD3D9:
        {
            ...
            ptrRenderer = new VideoRenderDirect3D9(
                    NULL, (HWND) _prtWindow, _fullscreen);
            if (ptrRenderer == NULL) {
                break;
            }
        ...
        case kVideoRenderWinMF:
        {
            ...
            ptrRenderer = new VideoRenderWinRT(
                    NULL, (HWND)_prtWindow, _fullscreen);
            if (ptrRenderer == NULL) {
                break;
            }
...
uint32_t VideoRenderWindowsImpl::RenderFrameRate(const uint32_t streamId) {
    CriticalSectionScoped cs(&_renderWindowsCritsect);
    return 0;
}
...
int32_t VideoRenderWindowsImpl::SetTransparentBackground(const bool enable) {
    CriticalSectionScoped cs(&_renderWindowsCritsect);
    int32_t error = -1;
    if (!_ptrRendererWin) {
    } else {
