...
int32_t AudioDeviceModuleImpl::CreatePlatformSpecificObjects() {
    ...
    if (audioLayer == kWindowsWasapiAudio) {
      WEBRTC_TRACE(kTraceInfo, kTraceAudioDevice, _id,
          "attempting to use the Windows Wasapi Audio APIs...");

        // create *Windows Core Audio* implementation
      ptrAudioDevice = new AudioDeviceWindowsWasapi(Id());
        WEBRTC_TRACE(kTraceInfo, kTraceAudioDevice, _id,
            "Windows Wasapi Audio APIs will be utilized");
    }
#else
    if ((audioLayer == kWindowsCoreAudio) ||
        ...
        if (AudioDeviceWindowsCore::CoreAudioIsSupported()) {
            ...
            if (ptrAudioDevice != NULL) {
                // Core Audio was not supported => revert to Windows Wave
                // instead modify the state set at construction
                _platformAudioLayer = kWindowsWaveAudio;
                WEBRTC_TRACE(kTraceWarning, kTraceAudioDevice, _id,
                "Windows Core Audio is *not* supported =>\
                Wave APIs will be utilized instead");
            }
        }
    }
#endif  // defined(WINRT)
    ...
    if ((audioLayer == kLinuxPulseAudio) ||
        ...
        if (pulseDevice->Init() != -1) {
            ...
            if (ptrAudioDevice != NULL) {
                ...
                WEBRTC_TRACE(kTraceWarning, kTraceAudioDevice, _id,
                    "Linux PulseAudio is *not* supported => ALSA APIs will\
                     be utilized instead");
            }
#endif
#if defined(LINUX_PULSE)
    ...
    if (ptrAudioDevice == NULL) {
        WEBRTC_TRACE(kTraceCritical, kTraceAudioDevice, _id,
            "unable to create the platform specific audio\
...
int32_t AudioDeviceModuleImpl::WaveOutVolume(uint16_t* volumeLeft,
    ...
    *volumeRight = volRight;
    ...
    WEBRTC_TRACE(kTraceStateInfo, kTraceAudioDevice, _id,
        "outputs: volumeLeft=%u, volumeRight=%u",
...
int32_t AudioDeviceModuleImpl::SetPlayoutBuffer(const BufferType type,
    ...
    if (_ptrAudioDevice->PlayoutIsInitialized()) {
        WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id,
            "unable to modify the playout buffer while playing\
             side is initialized");
        return -1;
    }
...
AudioDeviceModule::AudioLayer
    AudioDeviceModuleImpl::PlatformAudioLayer() const {
