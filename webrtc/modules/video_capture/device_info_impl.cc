...
DeviceInfoImpl::DeviceInfoImpl(const int32_t id)
    : _id(id), _apiLock(*RWLockWrapper::CreateRWLock()),
                ...
                if ((diffWidth >= 0 && diffWidth <= abs(currentbestDiffWith))
                    ...
                    if (diffWidth == currentbestDiffWith && diffHeight
                        ...
                        if (((diffFrameRate >= 0 &&
                            ...
                            // Same frame rate as previous or frame rate
                            ...
                            if ((currentbestDiffFrameRate == diffFrameRate)
                                ...
                                // If width height and frame rate is full
                                // filled we can use the camera for encoding
                                // if it is supported.
...
int32_t DeviceInfoImpl::GetExpectedCaptureDelay(
    ...
    for (uint32_t device = 0; device < sizeOfDelayValues; ++device) {
        if (delayValues[device].productId &&
            ...
            for (uint32_t delayIndex = 0;
              delayIndex < NoOfDelayValues;
              ++delayIndex) {
                ...
                if ((diffHeight >= 0 &&
                      diffHeight <= abs(currentbestDiffHeight))  // Height better or equal than previous.
