...
int32_t MP4Recorder::StartRecordingVideoFile(
                                             const char* fileName,
                                             const CodecInst& audioCodecInst,
                                             const VideoCodec& videoCodecInst,
                                             ACMAMRPackingFormat amrFormat,
...
int32_t MP4Recorder::StopRecording()
{
    _timeEvent.StopTimer();
...
int32_t MP4Recorder::CalcI420FrameSize( ) const
{
    return 3 * _videoCodecInst.width * _videoCodecInst.height / 2;
}

int32_t MP4Recorder::SetUpVideoEncoder()
{
    _videoMaxPayloadSize = 0;
    return 0;
}

int32_t MP4Recorder::RecordVideoToFile(const I420VideoFrame& videoFrame)
{
    CriticalSectionScoped lock(_critSec);
...
int32_t MP4Recorder::ProcessAudio()
{
    if (_writtenVideoFramesCounter == 0)
    {
        // Get the most recent frame that is due for writing to file. Since
...
int32_t MP4Recorder::EncodeAndWriteVideoToFile(I420VideoFrame& videoFrame)
{
    if(!IsRecording())
    {
        return -1;
...
int32_t MP4Recorder::WriteEncodedAudioData(
                                           const int8_t* audioBuffer,
                                           uint16_t bufferLength,
                                           uint16_t millisecondsOfData,
                                           const TickTime* playoutTS)
