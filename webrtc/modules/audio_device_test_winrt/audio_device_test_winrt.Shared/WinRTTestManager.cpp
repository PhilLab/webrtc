  ...
  uint32_t& newMicLevel)
{
  ...
  if (_recCount % 100 == 0)
  {
    ...
    if (_microphoneVolume)
    {
      ...
      //_audioDevice->MaxMicrophoneVolume(&maxVolume);
      //_audioDevice->MinMicrophoneVolume(&minVolume);
      //_audioDevice->MicrophoneVolumeStepSize(&stepSize);
      //_audioDevice->MicrophoneVolume(&volume);
      ...
      //_audioDevice->SetMicrophoneVolume(volume);
    }
    ...
    if (_microphoneMute && (_recCount % 500 == 0))
    {
      ...
      //_audioDevice->MicrophoneMute(&muted);
      muted = !muted;
       EXPECT_EQ(0, _audioDevice->SetMicrophoneMute(muted));
      //_audioDevice->SetMicrophoneMute(muted);
      if (muted)
      {
        TEST_LOG("[MUTE ON]");
        addMarker = false;
    ...
    if (_microphoneBoost && (_recCount % 500 == 0))
    {
      ...
      //_audioDevice->MicrophoneBoost(&boosted);
      boosted = !boosted;
      EXPECT_EQ(0, _audioDevice->SetMicrophoneBoost(boosted));
      //_audioDevice->SetMicrophoneBoost(boosted);
      if (boosted)
      {
        TEST_LOG("[BOOST ON]");
        addMarker = false;
    ...
    else if ((nChannels == 2) && (nBytesPerSample == 2) && addMarker)
    {
      ...
      //_audioDevice->RecordingChannel(&chType);
  ...
  int64_t* ntp_time_ms)
{
  ...
  if (_playCount % 100 == 0)
  {
    ...
    if (_speakerVolume)
    {
      ...
      //_audioDevice->MaxSpeakerVolume(&maxVolume);
      //_audioDevice->MinSpeakerVolume(&minVolume);
      //_audioDevice->SpeakerVolumeStepSize(&stepSize);
      //_audioDevice->SpeakerVolume(&volume);
      if (volume == 0)
      {
        TEST_LOG("[0]");
        addMarker = false;
      ...
      //_audioDevice->SetSpeakerVolume(volume);
    }
    ...
    if (_speakerMute && (_playCount % 500 == 0))
    {
      ...
      //_audioDevice->SpeakerMute(&muted);
      muted = !muted;
      EXPECT_EQ(0, _audioDevice->SetSpeakerMute(muted));
      //_audioDevice->SetSpeakerMute(muted);
      if (muted)
      {
        TEST_LOG("[MUTE ON]");
        addMarker = false;
    ...
    if (_loopBackMeasurements)
    {
      ...
      //_audioDevice->RecordingDelay(&recDelayMS);
      //_audioDevice->PlayoutDelay(&playDelayMS);
...
int32_t WinRTTestManager::Init()
{
  ...
  //_audioDevice = webrtc::AudioDeviceModuleImpl::Create(555, ADM_AUDIO_LAYER);
