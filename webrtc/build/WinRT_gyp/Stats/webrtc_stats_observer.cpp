...
WebRTCStatsObserver::WebRTCStatsObserver(
  ...
  conn_health_stats_observer_(NULL), etw_stats_enabled_(false) {
...
void WebRTCStatsObserver::ToggleConnectionHealthStats(
  ConnectionHealthStatsObserver* observer) {
  conn_health_stats_observer_ = observer;
  LOG(LS_INFO) << "WebRTCStatsObserver "
               << (conn_health_stats_observer_? "enabling" : "disabling")
...
void WebRTCStatsObserver::OnComplete(const StatsReports& reports) {
  for (auto report : reports) {
    ...
    if (conn_health_stats_observer_) {
  ...
  if (conn_health_stats_observer_) {
    ...
    conn_health_stats_observer_->OnConnectionHealthStats(conn_health_stats_);
  }
}
...
void WebRTCStatsObserver::EvaluatePollNecessity() {
  if (etw_stats_enabled_ || conn_health_stats_observer_) {
    Start();
  } else {
    Stop();
  }
