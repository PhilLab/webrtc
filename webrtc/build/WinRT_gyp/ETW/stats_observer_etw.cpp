...
#include "webrtc/build/WinRT_gyp/ETW/etw_providers.h"  // generated by message compiler

namespace webrtc {

const int StatsObserverETW::kInterval = 1000;  //  The time interval of requesting statistics
enum {
  MSG_POLL_STATS,
};
  ...
  for (auto report : reports) {
    ...
    for (auto value : report->values()) {
      ...
      switch (value.second->type()) {
        ...
        EventWriteStatsReportInt32(stat_group_name, timestamp, stat_name, value.second->int_val());
        break;
      case StatsReport::Value::kInt64:
        EventWriteStatsReportInt64(stat_group_name, timestamp, stat_name, value.second->int64_val());
        break;
      case StatsReport::Value::kFloat:
        EventWriteStatsReportFloat(stat_group_name, timestamp, stat_name, value.second->float_val());
        break;
      case StatsReport::Value::kBool:
        EventWriteStatsReportBool(stat_group_name, timestamp, stat_name, value.second->bool_val());
        break;
      case StatsReport::Value::kStaticString:
        EventWriteStatsReportString(stat_group_name, timestamp, stat_name, value.second->static_string_val());
        break;
      case StatsReport::Value::kString:
        EventWriteStatsReportString(stat_group_name, timestamp, stat_name, value.second->string_val().c_str());
        break;
      default:
        break;
      }
...
void StatsObserverETW::PollStats(rtc::scoped_refptr<webrtc::PeerConnectionInterface> pci) {
  if (!pci.get()) {
    return;
  }
...
void StatsObserverETW::GetStreamCollectionStats(rtc::scoped_refptr<StreamCollectionInterface> streams) {
  ...
  for (size_t i = 0; i < ss_count; ++i) {
    ...
    for (auto audio_track : audio_tracks) {
      pci_->GetStats(this, audio_track, PeerConnectionInterface::kStatsOutputLevelDebug);
    }

    for (auto video_track : video_tracks) {
      pci_->GetStats(this, video_track, PeerConnectionInterface::kStatsOutputLevelDebug);
    }
  }
}
