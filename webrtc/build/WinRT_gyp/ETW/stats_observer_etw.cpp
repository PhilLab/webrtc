...
StatsObserverETW::StatsObserverETW() : pci_(NULL), status_(kDisabled),
...
void StatsObserverETW::Stop() {
  CriticalSectionScoped lock(crit_sect_.get());
  status_ = kDisablePending;
  LOG(LS_INFO) << "StatsObserverETW stopping";
}
...
void StatsObserverETW::PollStats() {
...

  {
    CriticalSectionScoped lock(crit_sect_.get());
    if (status_ != kEnabled) {
      LOG(LS_INFO) << "StatsObserverETW stopped";
      status_ = kDisabled;
      return;
    }
