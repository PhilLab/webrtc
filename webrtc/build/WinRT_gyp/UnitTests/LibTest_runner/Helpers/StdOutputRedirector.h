 ...
 private:
    ...
    template<> struct BufferT < true > {
      ...
      ~BufferT() {
        ...
        int nRequiredSize = ::MultiByteToWideChar(CP_ACP, 0, const_cast<char*>(
            buffer.c_str()), -1, NULL, 0);
        if (nRequiredSize >= 0) {
          output.resize(nRequiredSize - 1);  // without ending NULL
          nRequiredSize = ::MultiByteToWideChar(CP_ACP, 0, const_cast<char*>(
              buffer.c_str()), nRequiredSize - 1, const_cast<wchar_t*>(
              output.c_str()), nRequiredSize);
        }
      }
    };
    // buffer type
