...
template <class T> struct CStonCreateStatic {
    ...
    //! This macro is a helper for easy and fast definition of the singleton
    //! creation policy for cases when Constructor has more arguments
...
//=============================================================================
//! \brief This is a lifetime policy of class for CSafeSingletonT.
//! It maintains its own list, of instances. This is usefull for example
// in DLLs.
//=============================================================================
    ...
    class CDestroyItem {
     ...
     public:
        ...
        static void DestroyItems() {
            // lock this for whole list iteration,
            // so there is not added Phoenix into list
            // xpl::CMutexLock   lock(CXplLibrary::InitMutex());

            // call termination handlers
            while (*GetHead() != NULL)
                // invoking the destructor causes the termination
