...
#pragma warning( disable : 4290 )
  ...
  XmlElement^ CXmlReporter::GetLibraryElement(String^ libraryName)
  {
    ...
    XmlNodeList^ libraries = solutionEl_->GetElementsByTagName(LIBRARY_ELEMENT_NAME);
  ...
  //    Description: Gets specified project element in specified library element.
  ...
  // 2015/03/16 TP: created
  //======================================================================
  XmlElement^ CXmlReporter::GetProjectElement(XmlElement^ library, String^ projectName)
  {
    XmlElement^ ret;

    XmlNodeList^ projects = library->GetElementsByTagName(PROJECT_ELEMENT_NAME);
    if (projects != nullptr)
    {
  ...
  void CXmlReporter::AddTestSolutionHeader(const CTestSolution& solution) throw(ReportGenerationException)
  {
  ...
  {
    auto writeTask = create_task(ApplicationData::Current->LocalFolder->CreateFileAsync(OutputFile_, CreationCollisionOption::ReplaceExisting));
