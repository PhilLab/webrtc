...
int libSrtpTests::CSrtpAesCalcTest::InterchangeableExecute()
{
  ...
  char* argv[] = { ".", "000102030405060708090a0b0c0d0e0f", "00112233445566778899aabbccddeeff", "-v" };
...
void libSrtpTests::CSrtpAesCalcTest::InterchangeableVerifyResult()
{
  ...
  if (Succeed())
  {
    size_t begin =m_wsOutput.find_first_not_of(L" :\t", Output().rfind(kCipherTextKey) + wcslen(kCipherTextKey));
    size_t end = m_wsOutput.find_first_of(L" \n", begin);
    std::wstring cipherText(m_wsOutput.cbegin() + begin, m_wsOutput.cbegin() + end);

    if (cipherText.compare(CIPHER_TEXT) != 0)
    {
      SetSucceed(false);
      m_wsResultMessage += L"ciphertext doesn't match. Expected: " CIPHER_TEXT L"\n";
    }
