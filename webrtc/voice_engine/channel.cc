                  ...
                  const RTPFragmentationHeader* fragmentation)
{
    ...
    assert(_channelMode != kReceiveForwardingChannel);

    if (_includeAudioLevelIndication)
    {
        assert(_channelMode != kSendForwardingChannel);
                              ...
                              uint8_t volume)
{
    ...
    assert(_channelMode == kFullChannel);
                               ...
                               const WebRtcRTPHeader* rtpHeader)
{
    ...
    if (_channelMode == kFullChannel && !channel_state_.Get().playing)
    {
    ...
    if (_channelMode == kFullChannel &&
        audio_coding_->IncomingPacket(payloadData,
                                      payloadSize,
                                      *rtpHeader) != 0)
    {
        _engineStatisticsPtr->SetLastError(
                       ...
                       ChannelMode channelMode)
{
    WEBRTC_TRACE(kTraceMemory, kTraceVoice, VoEId(instanceId,channelId),
                 "Channel::CreateChannel(channelId=%d, instanceId=%d, channelMode=%d)",
        channelId, instanceId, channelMode);

    channel = new Channel(channelId, instanceId, config, channelMode);
    if (channel == NULL)
    {
...
Channel::Channel(int32_t channelId,
    ...
    rtp_header_parser_(channelMode == kSendForwardingChannel ? NULL : RtpHeaderParser::Create()),
    rtp_payload_registry_(
        channelMode == kSendForwardingChannel ? NULL :
        new RTPPayloadRegistry(channelId,
            RTPPayloadStrategy::CreateStrategy(true))),
    rtp_receive_statistics_(ReceiveStatistics::Create(
        Clock::GetRealTimeClock())),
    rtp_receiver_(channelMode == kSendForwardingChannel ? NULL : RtpReceiver::CreateAudioReceiver(
        VoEModuleId(instanceId, channelId), Clock::GetRealTimeClock(), this,
        this, this, rtp_payload_registry_.get())),
    telephone_event_handler_(channelMode == kSendForwardingChannel ? NULL : rtp_receiver_->GetTelephoneEventHandler()),
    audio_coding_((channelMode == kFullChannel || channelMode == kReceiveForwardingChannel) ? config.Get<AudioCodingModuleFactory>().Create(
        VoEModuleId(instanceId, channelId)) : NULL),
    ...
    _transportPtr(NULL),
    rx_audioproc_(channelMode == kFullChannel ? AudioProcessing::Create(
        VoEModuleId(instanceId, channelId)) : NULL),
...
Channel::~Channel()
{
    ...
    // 3. Destroy modules
    if (_channelMode == kFullChannel && audio_coding_->RegisterTransportCallback(NULL) == -1)
    {
        WEBRTC_TRACE(kTraceWarning, kTraceVoice,
                     VoEId(_instanceId,_channelId),
    ...
    if (_channelMode == kFullChannel && audio_coding_->RegisterVADCallback(NULL) == -1)
    {
        WEBRTC_TRACE(kTraceWarning, kTraceVoice,
                     VoEId(_instanceId,_channelId),
...
Channel::Init()
{
    ...
    if ((_channelMode == kFullChannel || _channelMode == kReceiveForwardingChannel) && ((audio_coding_->InitializeReceiver() == -1) ||
    ...
    if (_channelMode == kFullChannel || _channelMode == kReceiveForwardingChannel)
    ...
    for (int idx = 0; idx < nSupportedCodecs; idx++)
    {
        ...
        if (!STR_CASE_CMP(codec.plname, "telephone-event"))
        {
            if ((_rtpRtcpModule->RegisterSendPayload(codec) == -1) ||
                ((_channelMode == kFullChannel || _channelMode == kReceiveForwardingChannel) &&
    ...
    if ((_channelMode == kFullChannel) && (rx_audioproc_->noise_suppression()->set_level(kDefaultNsMode) != 0)) {
      LOG_FERR1(LS_ERROR, noise_suppression()->set_level, kDefaultNsMode);
      return -1;
    }
    if ((_channelMode == kFullChannel) && (rx_audioproc_->gain_control()->set_mode(kDefaultRxAgcMode) != 0)) {
...
Channel::StartPlayout()
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::StartPlayout() cannot start playout on forwarding channel");
        return -1;
    }
...
Channel::StopPlayout()
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::StopPlayout() cannot stop playout on forwarding channel");
        return -1;
    }
...
Channel::StartSend()
{
    ...
    if (_channelMode == kReceiveForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::StartSend() cannot start sending on receive forwarding channel");
        return -1;
    }
...
Channel::StopSend()
{
    ...
    if (_channelMode == kReceiveForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::StopSend() cannot stop sending on receive forwarding channel");
        return -1;
    }
...
Channel::StartReceiving()
{
    ...
    if (_channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::StartReceiving() cannot start receiving on forwarding channel");
        return -1;
    }
...
Channel::StopReceiving()
{
    ...
    if (_channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::StopReceiving() cannot stop receiving on forwarding channel");
        return -1;
    }
    if (!channel_state_.Get().receiving)
    {
...
Channel::SetNetEQPlayoutMode(NetEqModes mode)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::SetNetEQPlayoutMode() cannot set NetEQ playout mode on forwarding channel");
        return -1;
    }
...
Channel::GetNetEQPlayoutMode(NetEqModes& mode)
{
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::GetNetEQPlayoutMode() cannot get NetEQ playout mode for forwarding channel");
        return -1;
    }
...
Channel::GetSendCodec(CodecInst& codec)
{
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::GetSendCodec() get send codec method is not supported for forwarding channel");
        return -1;
    }
...
Channel::GetRecCodec(CodecInst& codec)
{
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::GetRecCodec() cannot get receive codec for forwarding channel");
        return -1;
    }
...
Channel::SetSendCodec(const CodecInst& codec)
{
    ...
    if ((_channelMode == kFullChannel) && (audio_coding_->RegisterSendCodec(codec) != 0))
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice, VoEId(_instanceId,_channelId),
...
Channel::SetVADStatus(bool enableVAD, ACMVADMode mode, bool disableDTX)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::SetVADStatus() cannot set VAD status on forwarding channel");
        return -1;
    }
...
Channel::GetVADStatus(bool& enabledVAD, ACMVADMode& mode, bool& disabledDTX)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::GetVADStatus() cannot get VAD status for forwarding channel");
        return -1;
    }
...
Channel::SetRecPayloadType(const CodecInst& codec)
{
    ...
    if (_channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::SetRecPayloadType() cannot set receive payload type on send forwarding channel");
        return -1;
    }
    if (channel_state_.Get().playing)
    {
...
Channel::GetRecPayloadType(CodecInst& codec)
{
    ...
    if (_channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::GetRecPayloadType() cannot get receive payload type for forwarding channel");
        return -1;
    }
...
Channel::SetSendCNPayloadType(int type, PayloadFrequencies frequency)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::SetSendCNPayloadType() CN payload type cannot be set on forwarding channel");
        return -1;
    }
...
Channel::SetISACInitTargetRate(int rateBps, bool useFixedFrameSize)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::SetISACInitTargetRate() ISAC init target rate cannot be set on forwarding channel");
        return -1;
    }
...
Channel::SetISACMaxRate(int rateBps)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::SetISACMaxRate() ISAC max rate cannot be set on forwarding channel");
        return -1;
    }
...
Channel::SetISACMaxPayloadSize(int sizeBytes)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::SetISACMaxPayloadSize() ISAC max payload size cannot be set on forwarding channel");
        return -1;
    }
    CodecInst sendCodec;
    if (audio_coding_->SendCodec(&sendCodec) == -1)
...
int32_t Channel::ReceivedRTPPacket(const int8_t* data, int32_t length,
  ...
  if (_channelMode == kSendForwardingChannel)
  {
    WEBRTC_TRACE(kTraceError, kTraceVoice,
                 VoEId(_instanceId,_channelId),
                 "Channel::ReceivedRTPPacket() send forwarding channel cannot receive RTP packets");
    return -1;
  }
                                     ...
                                     const CodecInst* codecInst)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::StartPlayingFileLocally() cannot start playing file locally on forwarding channel");
        return -1;
    }
...
int Channel::StopPlayingFileLocally()
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::StopPlayingFileLocally() cannot stop playing file locally on forwarding channel");
        return -1;
    }
                                          ...
                                          const CodecInst* codecInst)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::StartPlayingFileAsMicrophone() cannot start playing file as a microphone on forwarding channel");
        return -1;
    }
...
int Channel::StopPlayingFileAsMicrophone()
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::StopPlayingFileAsMicrophone() cannot stop playing file as a microphone on forwarding channel");
        return -1;
    }
                                   ...
                                   const CodecInst* codecInst)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::StartRecordingPlayout() cannot start recording playout on forwarding channel");
        return -1;
    }
...
int Channel::StopRecordingPlayout()
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::StopRecordingPlayout() cannot stop recording playout on forwarding channel");
        return -1;
    }
...
Channel::SetDtmfPlayoutStatus(bool enable)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::SetDtmfPlayoutStatus() cannot set DTMF playout status on forwarding channel");
        return -1;
    }
...
Channel::UpdateRxVadDetection(AudioFrame& audioFrame)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::UpdateRxVadDetection() VAD detection is not supported on forwarding channel");
        return -1;
    }
...
Channel::RegisterRxVadObserver(VoERxVadCallback &observer)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::RegisterRxVadObserver() VAD observer cannot be registered on forwarding channel");
        return -1;
    }
...
Channel::DeRegisterRxVadObserver()
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::DeRegisterRxVadObserver() VAD observer cannot be deregistered on forwarding channel");
        return -1;
    }
...
Channel::SetRxAgcStatus(bool enable, AgcModes mode)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::SetRxAgcStatus() AGC status cannot be set on forwarding channel");
        return -1;
    }
...
Channel::GetRxAgcStatus(bool& enabled, AgcModes& mode)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::GetRxAgcStatus() AGC status cannot be retrieved for forwarding channel");
        return -1;
    }
...
Channel::SetRxAgcConfig(AgcConfig config)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::SetRxAgcConfig() AGC configuration cannot be set on forwarding channel");
        return -1;
    }
...
Channel::GetRxAgcConfig(AgcConfig& config)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::GetRxAgcConfig() AGC configuration cannot be retrieved from forwarding channel");
        return -1;
    }
...
Channel::SetRxNsStatus(bool enable, NsModes mode)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::SetRxNsStatus() cannot set NS status on forwarding channel");
        return -1;
    }
...
Channel::GetRxNsStatus(bool& enabled, NsModes& mode)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::SetRxNsStatus() cannot get NS status for forwarding channel");
        return -1;
    }
...
Channel::SetLocalSSRC(unsigned int ssrc)
{
    ...
    if (_channelMode == kReceiveForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::SetLocalSSRC() cannot set local SSRC for receive forwarding channel");
        return -1;
    }
    if (channel_state_.Get().sending)
    {
...
Channel::GetLocalSSRC(unsigned int& ssrc)
{
    if (_channelMode == kReceiveForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::GetLocalSSRC() cannot get local SSRC for receive forwarding channel");
        return -1;
    }
...
Channel::GetRemoteSSRC(unsigned int& ssrc)
{
    if (_channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::GetRemoteSSRC() cannot get remote SSRC for send forwarding channel");
        return -1;
    }
...
Channel::GetRemoteCSRCs(unsigned int arrCSRC[15])
{
    if (_channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::GetRemoteCSRCs() cannot get remote CSRCs for send forwarding channel");
        return -1;
    }
    if (arrCSRC == NULL)
    {
...
int Channel::SetSendAudioLevelIndicationStatus(bool enable, unsigned char id) {
  if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
  {
    WEBRTC_TRACE(kTraceError, kTraceVoice,
                 VoEId(_instanceId,_channelId),
                 "Channel::SetSendAudioLevelIndicationStatus() cannot set audio level indication status on forwarding channel");
    return -1;
  }
...
int Channel::SetSendAbsoluteSenderTimeStatus(bool enable, unsigned char id) {
  if (_channelMode == kReceiveForwardingChannel)
  {
    WEBRTC_TRACE(kTraceError, kTraceVoice,
                 VoEId(_instanceId,_channelId),
                 "Channel::SetSendAbsoluteSenderTimeStatus() cannot set send absolute sender time status on receive forwarding channel");
    return -1;
  }
...
int Channel::SetReceiveAbsoluteSenderTimeStatus(bool enable, unsigned char id) {
  if (_channelMode == kSendForwardingChannel)
  {
    WEBRTC_TRACE(kTraceError, kTraceVoice,
                 VoEId(_instanceId,_channelId),
                 "Channel::SetReceiveAbsoluteSenderTimeStatus() cannot set receive absolute sender time status on send forwarding channel");
    return -1;
  }
...
Channel::GetRemoteRTCP_CNAME(char cName[256])
{
    if (_channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::GetRemoteRTCP_CNAME() cannot get remote CNAME for send forwarding channel");
        return -1;
    }
    if (cName == NULL)
    {
    ...
    unsigned short* fractionLost)
{
    ...
    if (_channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::GetRemoteRTCPData() cannot get remote RTCP data for send forwarding channel");
        return -1;
    }
                                             ...
                                             unsigned short dataLengthInBytes)
{
    ...
    if (_channelMode == kReceiveForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::SendApplicationDefinedRTCPPacket() cannot send application defined RTCP packet for receive forwarding channel");
        return -1;
    }
    if (!channel_state_.Get().sending)
    {
        ...
        unsigned int& discardedPackets)
{
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::GetRTPStatistics() cannot get RTP statistics for forwarding channel");
        return -1;
    }
...
Channel::GetRTPStatistics(CallStatistics& stats)
{
    if (_channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::GetRTPStatistics() cannot get RTP statistics for send forwarding channel");
        return -1;
    }
...
int Channel::SetFECStatus(bool enable, int redPayloadtype) {
  ...
  if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
  {
    WEBRTC_TRACE(kTraceError, kTraceVoice,
                 VoEId(_instanceId,_channelId),
                 "Channel::SetFECStatus() cannot set FEC status on forwarding channel");
    return -1;
  }
...
Channel::GetFECStatus(bool& enabled, int& redPayloadtype)
{
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::GetFECStatus() cannot get FEC status for forwarding channel");
        return -1;
    }
...
void Channel::SetNACKStatus(bool enable, int maxNumberOfPackets) {
  if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
  {
    WEBRTC_TRACE(kTraceError, kTraceVoice,
                 VoEId(_instanceId,_channelId),
                 "Channel::SetNACKStatus() cannot set NACK status on forwarding channel");
    return;
  }
...
int Channel::ResendPackets(const uint16_t* sequence_numbers, int length) {
  if (_channelMode == kSendForwardingChannel)
  {
    WEBRTC_TRACE(kTraceError, kTraceVoice,
                 VoEId(_instanceId,_channelId),
                 "Channel::ResendPackets() cannot send NACK on forwarding channel");
    return -1;
  }
                      ...
                      RTPDirections direction)
{
    ...
    if (((direction != kRtpIncoming) && (direction != kRtpOutgoing)) ||
        (_channelMode == kReceiveForwardingChannel && (direction != kRtpOutgoing)) ||
        (_channelMode == kSendForwardingChannel && (direction != kRtpIncoming)))
    {
        _engineStatisticsPtr->SetLastError(
            VE_INVALID_ARGUMENT, kTraceError,
...
Channel::Demultiplex(const AudioFrame& audioFrame)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::Demultiplex() demultiplex cannot be called on forwarding channel");
        return -1;
    }
...
Channel::PrepareEncodeAndSend(int mixingFrequency)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::PrepareEncodeAndSend() should not be used on forwarding channel");
        return -1;
    }
...
Channel::EncodeAndSend()
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::EncodeAndSend() should not be used on forwarding channel");
        return -1;
    }
    ...
    VoEMediaProcess& processObject)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::RegisterExternalMediaProcessing() external media processing cannot be registered on forwarding channel");
        return -1;
    }
...
int Channel::DeRegisterExternalMediaProcessing(ProcessingTypes type)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::RegisterExternalMediaProcessing() external media processing cannot be deregistered from forwarding channel");
        return -1;
    }
...
int Channel::SetExternalMixing(bool enabled) {
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::SetExternalMixing() external mixing cannot be set on forwarding channel");
        return -1;
    }
...
Channel::ResetRTCPStatistics()
{
    ...
    if (_channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::ResetRTCPStatistics() remote RTCP statistics cannot be reset on send forwarding channel");
        return -1;
    }
...
Channel::GetRoundTripTimeSummary(StatVal& delaysMs) const
{
    ...
    if (_channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::GetRoundTripTimeSummary() there is no RTT summary for send forwarding channel");
        return -1;
    }
...
Channel::GetNetworkStatistics(NetworkStatistics& stats)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::GetNetworkStatistics() network statistics cannot be obtained for forwarding channel");
        return -1;
    }
...
void Channel::GetDecodingCallStatistics(AudioDecodingCallStats* stats) const {
  if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
  {
    WEBRTC_TRACE(kTraceError, kTraceVoice,
                 VoEId(_instanceId,_channelId),
                 "Channel::GetDecodingCallStatistics() decoding statistics cannot be obtained for forwarding channel");
    return;
  }
...
int Channel::SetInitialPlayoutDelay(int delay_ms)
{
  ...
  if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
  {
    WEBRTC_TRACE(kTraceError, kTraceVoice,
                 VoEId(_instanceId,_channelId),
                 "Channel::SetInitialPlayoutDelay() initial playout delay cannot be set for forwarding channel");
    return -1;
  }
...
Channel::SetMinimumPlayoutDelay(int delayMs)
{
    ...
    if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::SetMinimumPlayoutDelay() minimum playout delay cannot be set for forwarding channel");
        return -1;
    }
...
void Channel::UpdatePlayoutTimestamp(bool rtcp) {
  ...
  if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
  {
    WEBRTC_TRACE(kTraceError, kTraceVoice,
                 VoEId(_instanceId,_channelId),
                 "Channel::UpdatePlayoutTimestamp() playout timestamp cannot be updated on forwarding channel");
    return;
  }
...
int Channel::SetSecondarySendCodec(const CodecInst& codec,
  ...
  if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
  {
    WEBRTC_TRACE(kTraceError, kTraceVoice,
                 VoEId(_instanceId,_channelId),
                 "Channel::SetSecondarySendCodec() cannot set secondary send codec on forwarding channel");
    return -1;
  }
...
void Channel::RemoveSecondarySendCodec() {
  if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
  {
    WEBRTC_TRACE(kTraceError, kTraceVoice,
                 VoEId(_instanceId,_channelId),
                 "Channel::RemoveSecondarySendCodec() cannot remove secondary send codec on forwarding channel");
    return;
  }
...
int Channel::GetSecondarySendCodec(CodecInst* codec) {
  if (_channelMode == kReceiveForwardingChannel || _channelMode == kSendForwardingChannel)
  {
    WEBRTC_TRACE(kTraceError, kTraceVoice,
                 VoEId(_instanceId,_channelId),
                 "Channel::GetSecondarySendCodec() cannot get secondary send codec from forwarding channel");
    return -1;
  }
