...
#ifndef H264_COMPTRLIST_H_
#define H264_COMPTRLIST_H_
...
class List {
 protected:
    struct Node {
        ...
        explicit Node(T item) : prev(nullptr), next(nullptr) {
            this->item = item;
        }
 ...
 public:
    class POSITION {
     ...
     private:
        ...
        explicit POSITION(Node *p) : pNode(p) {
        }
    };
 ...
 public:
    ...
    // ppItem can be nullptr if you don't want the item back.
    // (But the method does not release the item.)
...
// succeed but return a nullptr pointer. By default,
// the list does not allow nullptr pointers.
